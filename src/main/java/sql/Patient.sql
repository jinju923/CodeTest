--PATIENT 테이블에서 12세 이하인 여자환자의 환자이름, 환자번호, 성별코드, 나이, 전화번호를 조회하는 SQL문을작성해주세요.
...
--CASE WHEN TLNO IS NULL THEN 'NONE' ELSE TLNO END AS TLNO 대신
-- NVL(TLNO, 'NONE') AS TLNO 로 사용 가능. (NVL구문)